...
const DropZoneArea = ({ className, children, ...props }: DropZoneAreaProps) => {
  ...
  const { onFocus, onBlur, onDragEnter, onDragLeave, onDrop } =
  ...
  return (
    <section
      ...
      onDrop={onDrop}
      aria-describedby={context.rootDescriptionId}
      aria-labelledby={context.rootMessageId}
      {...props}
      className={cn(
        "flex items-center justify-center rounded-md border border-input border-dashed bg-background ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 aria-disabled:opacity-50",
        context.isDragActive && "animate-pulse bg-black/5",
        context.isInvalid && "border-destructive border-solid",
        className,
      )}
    >
      {children}
...
const DropzoneFileListItem = ({
  className,
  ...props
}: DropzoneFileListItemProps<unknown, unknown> & ItemProps) => {
  ...
  (props, ref) => {
    ...
    return (
      <p
        ...
        {...rest}
        className={cn(
          "mt-2 h-5 font-medium text-[0.8rem] text-destructive",
          rest.className,
        )}
      >
        {body}
